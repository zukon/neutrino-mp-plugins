...
{
	memset(raw,0,132*64);
	memset(lcd,0,LCD_ROWS*LCD_COLS);
}
...
void LCD_draw_rectangle (int left,int top, int right, int bottom, int linestate,int fillstate)
{
	...
	LCD_draw_line(left,bottom,right+1,bottom,linestate);
...
void LCD_draw_Ichar(int x, int y, char c)
{
int ax,ay;
unsigned char *data=&font[8*c];
	...
	for (ay=0; ay<8; ay++)
		for (ax=0; ax<8; ax++) 
			if ( (!LCD_invalid_col(x+ax)) && (!LCD_invalid_row(y+ay)))
				if(data[ay]&(1<<(7-ax)))
					LCD_draw_point(x+ax, y+ay, PIXEL_INV);
}

void LCD_draw_Istring(int x, int y, char *string)
{
	while (*string)
	{
		LCD_draw_Ichar(x, y, *string++);
		x+=8;
	}
}
