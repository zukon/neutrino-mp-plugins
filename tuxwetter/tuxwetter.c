...
#ifdef MARTII
	int ix=m->act_entry,tind=m->act_entry, sbw=(m->num_entrys>10)?14:0;
#else
...
//	RenderBox(0, 0, ixw, iyw, GRID, CMCS);
	...
	//selectbar
#ifdef MARTII
	RenderBox(isx+2, isy+moffs+toffs+soffs+(ix%10)*dy+2, ixw-sbw-2, dy+2, radius, CMCS);
#else
...
#ifdef MARTII
		scrollbar_ofs = scrollbar_len*(double)((ix/LIST_STEP)*LIST_STEP);
#else
...
//				memcpy(lfb, lbb, var_screeninfo.xres*var_screeninfo.yres);
				...
				if(show_icons)
				{
					...
					if(!slim)
#ifdef MARTII
						pic_on_data(icon,700, 115, 100, 100, 5, 3, 0, 0);
					else
...
#ifdef WWEATHER
#ifdef MARTII
					prs_get_dwday(ix-1, PRE_DAY,rstr);
#else
...
//				memcpy(lfb, lbb, var_screeninfo.xres*var_screeninfo.yres);
				...
				if(show_icons)
				{
					//tret=pic_on_data(icon, 540, 115, 100, 100, 5, 3, 0, 0);
#ifdef MARTII
					pic_on_data(icon,slim?540:700, 115, 100, 100, 5, 3, 0, 0);
#else
...
#ifdef MARTII
	*buffer=color_average_resize(*buffer,x1,y1,*imx,*imy);
#else
	tempx=*imx;
	tempy=*imy;
					...
					case TYP_CITY:
						if(!prs_get_city())
						{
...
#endif
							{
								...
								{
#ifdef MARTII
									prs_get_dwday(ix-2, PRE_DAY,tstr);
#else
