...
int fb_set_gmode(int gmode)
{
	...
	if(gmode)
	{
		...
		if(set332map() == -1)
		{
			printf("fb_display <FBIOPUTCMAP failed>\n");
			return -1;
		}
	}
	else
	{
		clearFB(720, 576, 8, 1);
		memcpy((void*)&var, (void*)&var_screeninfo, sizeof(struct fb_var_screeninfo));
		ioctl(fb, FBIOPUTCMAP, &map_back);
	}
#ifdef HAVE_DBOX_HARDWARE
	ioctl(fb, AVIA_GT_GV_SET_BLEV, (gmode)?0:alpha);
#endif
	if(ioctl (fb, FBIOPUT_VSCREENINFO,&var) == -1)
	{
		printf("fb_display <FBIOPUT_VSCREENINFO failed>\n");
		return -1;
	}
...
void fb_display(unsigned char *rgbbuff, int x_size, int y_size, int x_pan, int y_pan, int x_offs, int y_offs, int clearflag, int setpal)
{
    ...
    if((x_size < (int)var.xres || y_size < (int)var.yres)&& clearflag)
...
int x,y;
	...
	switch(cpp){
		case 2:
			...
			break;
		default:
			memset(lfb, TRANSP, stride*y*cpp);
	}
