...
setup_df::setup_df()
{
	...
	if (ismounted("/var/mnt/usb")) 
	{
		new eListBoxEntryText(mliste, "USB Stick", (void *)new eString("/var/mnt/usb")); 
		ok->show(); 
	}
	if (ismounted("/var/mnt/cf"))  
	{
		new eListBoxEntryText(mliste, "Compact Flash", (void *)new eString("/var/mnt/cf")); 
		ok->show(); 
	}
	if (ismounted("/hdd"))
	{
		new eListBoxEntryText(mliste, _("Harddisk"), (void *)new eString("/hdd")); 
		ok->show(); 
	}
	mliste->setCurrent(0);
...
info_df::info_df()
{
	...
	if (cfg.mpoint == "/var/mnt/usb")
		l->setText("USB Stick on \"/var/mnt/usb\"");
	else 
	if (cfg.mpoint == "/var/mnt/cf")
		l->setText("Compact Flash on \"/var/mnt/cf\"");
	else 
	if (cfg.mpoint == "/hdd")
		l->setText("Harddisk on \"/hdd\"");
	else
